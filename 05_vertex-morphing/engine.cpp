 ...
 public:
  ...
  bool load_texture(std::string path) final {
    ...
    if (error != 0) {
      ...
      std::cout << "Loaded image successfull";
    }
    GLuint tex_handl = 0;
    ...
    ENGINE_GL_CHECK();
    glTexImage2D(GL_TEXTURE_2D, 1, GL_RGBA, w, h, 0, GL_RGBA, 0,
                 &image[0]);  //разобраться в этой функции, она самая важная
    //начать с четырех пиксельной структуры для теста
    ENGINE_GL_CHECK();
    // TODO load texture to shader, create sample.
